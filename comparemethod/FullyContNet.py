# code from https://github.com/DotWang/FullyContNet/tree/main
# if you want to use it, please cite Fully Contextual Network for Hyperspectral Scene Parsing (TGRS 2021)
# Thanks for Di Wang, Bo Du, and Liangpei Zhang
...
if __name__ == '__main__':
    ...
    input = torch.rand(1, 200, 145,145 ).to('cuda:0') 
    ...
    # print(abu.shape) 
    # print(hsi_re.shape) 
    def measure_latency_cpu(model, inputs, device="cpu", runs=30, warmup=10):
        model.eval()
        model = model.to(device)
        inputs = inputs.to(device)  # 确保输入也到同一设备
        ...
        print(f"Latency on {device}: {(end-start)/runs*1000:.2f} ms")
    ...
    model2 = fucontnet(args, 200, 16)
    measure_latency_cpu(model2, input_tensor, device="cpu")
    # measure_latency(model, input_tensor, device="cpu")
    measure_latency(model, input, device="cuda:0")
    # 测试显存峰值
    measure_memory(model, input, device="cuda:0")
    # 测试吞吐量
    measure_throughput(model, input, device="cuda:0")
