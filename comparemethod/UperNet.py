...
class ResNet(nn.Module):
    def __init__(
        ...
        width_per_group = 64, firstchannel=None,replace_stride_with_dilation = None, norm_layer = None):
        ...
        self.conv1 = nn.Conv2d(firstchannel, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
...
class UPerNet(nn.Module):
    def __init__(self, num_classes,firstchannel):
        super(UPerNet, self).__init__()
        self.num_classes = num_classes
        self.backbone = ResNet.resnet50(firstchannel=firstchannel,replace_stride_with_dilation=[1,2,4])
...
if __name__ == '__main__':
    ...
    input = torch.rand(1,200,145, 145).to('cuda:1') # 确保在CUDA上 1, 191, 1280, 307/ 1, 200, 145, 145  / 1,144, 349,1905
    model = UPerNet(num_classes=16,firstchannel=200).to('cuda:1') # 确保模型在CUDA上
