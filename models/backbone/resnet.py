...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(191, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def _forward_impl(self, x):
        ...
        x = self.maxpool(x)
        x = self.layer1(x)
        # print('xxx111',x.shape)
        out.append(x)
        x = self.layer2(x)
        # print('xxx222',x.shape)
        out.append(x)
        x = self.layer3(x)
        # print('xxx333',x.shape)
        out.append(x)
        x = self.layer4(x)
        # print('xxx444',x.shape)
        out.append(x)
        return out
